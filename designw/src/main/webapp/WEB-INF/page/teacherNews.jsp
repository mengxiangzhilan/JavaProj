...
<div class="table-responsive">
    ...
    <c:if test="${!empty teacherList}">
        <table class="table table-condensed table-hover">
            ...
            <c:forEach items="${teacherList}" var="td">
            <tbody>
            <tr>
                <th>${td.teacherId}</th>
                <th>${td.teacherName}</th>
                <th>${td.teacherPhone}</th>
                <th>${td.collegeIiD}</th>
                <th><a href="${pageContext.request.contextPath}/page/updateteachernews/${td.teacherId}" >修改</a></th>
            </tr>
            </tbody>
            </c:forEach>
            <tfoot><td colspan="3" style="text-align: left">删除教师信息请在用户管理进行</td></tfoot>
        </table>
    </c:if>
</div>
</body>
